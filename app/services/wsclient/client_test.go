...
func TestClientRecordStartedPersistsRunningStateAndSendStartedSendsStarted(t *testing.T) {
	store := newClientTestStore(t)
	conn := newFakeConn()
	dialer := &fakeDialer{conn: conn}
	...
	conn.readCh <- helloAckEnvelope(hello.MessageID)
	waitFor(t, func() bool { return client.IsActive() }, "client to become active")
	requireNoError(t, client.RecordStarted(context.Background(), localtaskstore.TaskReceipt{TaskID: "task-1", ExecutionAttemptID: "attempt-1"}))
